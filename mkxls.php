...
$line = preg_replace ('/[\s]{2,}/', '^', trim($balbars_file_line_num));


$balbars_file_line_num_data = explode("^", trim($line));


print_r($balbars_file_line_num_data);
...
$balkazna_file_data_record_num = dbase_numrecords($balkazna_file_data);
  ...
  if ($row['BALANCE'] == trim($excel_result_balacc))
	...
	$DB_DAY = 0;
		...
		for ($z = 1; $z <= $balkazna_file_data_record_num; $z++)
		{
	
		$row_bal = dbase_get_record_with_names($balkazna_file_data, $z);
	
			if ($row_bal['BALANCE'] == $row['BALANCE'])
			{
	
			$DB_DAY = $DB_DAY + $row_bal['DB_DAY'];
	...
	$excel_cell_balkazna->activate; $excel_cell_balkazna->value = $DB_DAY;
